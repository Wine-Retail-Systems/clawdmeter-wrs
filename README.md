...
| **OpenCode** (sst/opencode)        | aktiv        | Tokens heute, Vergleich Vortag, optional Backend-Quota        |
...
|              Provider-Screens (Beispiel)              |              Empty-State              |
| :---------------------------------------------------: | :-----------------------------------: |
|        ![Provider screen](screenshots/usage.png)         | ![Empty state](screenshots/splash.png) |
|         Pro aktivem Provider ein Screen          |    Wenn nichts konfiguriert ist    |
...
eigenen Intervall (Anthropic 60s, Bedrock 60s, OpenCode 15s, Langdock 600s),
normalisiert das Ergebnis und schickt es als JSON-Sequenz über BLE an das
Gerät.
...
Minimal-Beispiel mit zwei Providern:
...
- **Was wird gezeigt**: 5h-Window-Auslastung + 7d-Window-Auslastung (beide
  als %), Reset-Timer, Pace-Indikator (langsamer/schneller als linear
  erwartet), Regen-Rate (% pro Minute).
...
- **Was wird gezeigt**: Tokens heute (Mitternacht bis jetzt), Vergleich
  zu gestern, optional die Quota-Auslastung des dahinterliegenden
  Backend-Providers.
...
```jsonl
{"p":"anthropic","n":"Claude","k":"pct_window","m1":42,"m2":18,"r1":7200,"r2":432000,"pace":1,"regen":2.5,"st":"ok","ok":true}
...
- **Splash**: animierter Clawd (oder Wine-Sprite). Mittlerer Knopf
  zykliert Animationen.
...
- **Bluetooth**: Verbindungsstatus, Geräte-MAC, Bond-Reset.
- **Empty-State**: wenn nichts konfiguriert ist, zeigt das Gerät
  „Keine Provider konfiguriert" + den Hinweis `clawdmeter-daemon setup`.
...
`./screenshot.sh out.png [port]` baut darauf auf und liefert ein PNG.
